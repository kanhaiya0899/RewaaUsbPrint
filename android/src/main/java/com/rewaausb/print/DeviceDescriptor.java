...
public class DeviceDescriptor {
    ...
    public static JSONObject fromDeviceConnection2(UsbDeviceConnection connection)
            throws IllegalArgumentException {
        byte[] buffer = new byte[LENGTH];
        Log.e("fromDeviceConnection2","1");
        int res = connection.controlTransfer(REQUEST_TYPE, REQUEST, REQ_VALUE, REQ_INDEX,
                buffer, LENGTH, TIMEOUT);
        Log.e("fromDeviceConnection2","res = "+res);
        return getDevicePIDVID(buffer);
    }
    ...
    private static JSONObject getDevicePIDVID(byte[] buffer) {
        if (buffer[0] != UsbHelper.DESC_SIZE_DEVICE || buffer[1] != UsbHelper.DESC_TYPE_DEVICE) {
            Log.e("getDevicePIDVID","err: Invalid device desc");
            throw new IllegalArgumentException("Invalid device descriptor");
        }
        ...
        try {
            Log.w("DeviceDescriptor l114", String.valueOf(productId));
            jsonObject.put("deviceClass", deviceClass);
            jsonObject.put("deviceSubclass", deviceSubclass);
            jsonObject.put("deviceProtocol", deviceProtocol);
            jsonObject.put("maxPacketSize", maxPacketSize);
            jsonObject.put("vendorId", vendorId);
            jsonObject.put("productId", productId);
            Log.i("getDevicePIDVID", "json final: "+jsonObject.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jsonObject;
